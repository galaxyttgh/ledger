...
const PaymentForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!supplierId) newErrors.supplierId = 'Please select a supplier';
    if (!bankAccountId) newErrors.bankAccountId = 'Please select a bank account';
    if (!amount || parseFloat(amount) <= 0) newErrors.amount = 'Valid amount is required';
    if (!paymentDate) newErrors.paymentDate = 'Payment date is required';
    if (parseFloat(amount) > 1000000000) newErrors.amount = 'Amount seems too high';
  ...
  const handleCancel = () => {
    if (supplierId || amount || billId || bankAccountId) {
      if (window.confirm('You have unsaved changes. Are you sure you want to leave?')) {
        navigate('/payments');
      }
    } else {
  ...
  const selectedSupplier = suppliers.find(s => s.id === parseInt(supplierId));
  const selectedBill = bills.find(b => b.id === parseInt(billId));
  const selectedBank = bankAccounts.find(b => b.id === parseInt(bankAccountId));
  const amountNum = parseFloat(amount) || 0;
      ...
      <div className="max-w-lg mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm p-4 lg:p-6">
          ...
          <div className="mb-4">
            ...
            {errors.bankAccountId && (
              <p className="mt-1 text-xs text-red-600">{errors.bankAccountId}</p>
            )}
          </div>
          ...
          {supplierId && amountNum > 0 && (
            <div className="mb-6 p-4 bg-red-50 rounded-xl border border-red-200">
              <h4 className="text-sm font-semibold text-red-900 mb-2">Payment Preview</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-red-700">Supplier</span>
                ...
                {selectedBank && (
                  <div className="flex justify-between">
                    <span className="text-red-700">Pay From</span>
                    <span className="font-medium text-red-900">
