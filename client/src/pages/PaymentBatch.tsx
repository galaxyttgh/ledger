...
const PaymentBatch = () => {
  ...
  const [suppliers, setSuppliers] = useState<any[]>([]);
  const [bills, setBills] = useState<any[]>([]);
  const [bankAccounts, setBankAccounts] = useState<any[]>([]); // ← CHANGED
  const [supplierId, setSupplierId] = useState('');
  const [bankAccountId, setBankAccountId] = useState(''); // ← CHANGED
  const [selectedBills, setSelectedBills] = useState<number[]>([]);
  const [paymentDate, setPaymentDate] = useState(new Date().toISOString().split('T')[0]);
  const [loading, setLoading] = useState(false);
  const [selectAll, setSelectAll] = useState(false);
  ...
  const fetchBanks = async () => {
    try {
      ...
      if (response.data.length > 0) setBankAccountId(response.data[0].id.toString());
    } catch (error) {
  ...
  const total = bills.filter(b => selectedBills.includes(b.id)).reduce((sum, b) => sum + Number(b.total), 0);
  const selectedSupplier = suppliers.find(s => s.id === parseInt(supplierId));
  const selectedBank = bankAccounts.find(b => b.id === parseInt(bankAccountId)); // ← CHANGED
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post('/payments/batch', { 
        ...
        bill_ids: selectedBills, 
        payment_date: paymentDate,
        bank_account_id: parseInt(bankAccountId) // ← CHANGED
      });
      toast.success(`Successfully paid ${selectedBills.length} bill(s)`);
      setSelectedBills([]);
      setSelectAll(false);
  ...
  return (
    <Layout>
      ...
      {/* Supplier, Bank & Date Selection */}
      <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"> {/* ← CHANGED: 3 columns */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Supplier</label>
            <select 
              value={supplierId} 
          ...
          {/* ← CHANGED: Bank account selector */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Pay From (Bank Account)</label>
            <select 
              value={bankAccountId} 
              ...
              <div>
                ...
                {selectedBank && (
                  <p className="text-xs text-blue-600 mt-1">
                    Paying from: {selectedBank.bank_name} — {selectedBank.name}
                  </p>
                )}
              </div>
              <button 
